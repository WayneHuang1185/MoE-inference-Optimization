...
bool llama_rpp_gpu_cache::configure(
        ...
        llama_rpp_gpu_reclaim_policy reclaim_policy,
        std::string * error) {
    stop();
    ...
    if (model->arch != LLM_ARCH_GEMMA4 || model->layers.empty()) {
        if (error) {
            *error = "Stage 4B typed expert cache currently supports Gemma 4 only";
        }
        stop();
        return false;
    }
    const auto & layer0 = model->layers.front();
    if (!layer0.ffn_gate_up_exps || !layer0.ffn_down_exps ||
            !layer0.ffn_down_exps_s) {
        if (error) {
            *error = "Gemma 4 expert tensors are incomplete";
        }
        stop();
        return false;
    }

    const uint64_t gate_up_bytes = source->component_bytes(
            0, 0, llama_rpp_expert_component::gate_up_weight);
    const uint64_t scale_bytes = source->component_bytes(
            0, 0, llama_rpp_expert_component::down_scale);
    ...
    for (size_t layer = 0; layer < model->layers.size(); ++layer) {
        ...
        if (!model_layer.ffn_gate_up_exps || !model_layer.ffn_down_exps ||
            ...
            if (error) {
                *error = "Gemma 4 layer has incomplete expert tensors";
            }
            stop();
            return false;
        }
        if (model_layer.ffn_gate_up_exps->type != layer0.ffn_gate_up_exps->type ||
            ...
            if (error) {
                *error = "Gemma 4 gate_up/scale cache layout varies by layer";
            }
            stop();
            return false;
        }
        ...
        if (mapped_gate_up != model_layer.ffn_gate_up_exps->nb[2] ||
            ...
            if (error) {
                *error = "page-map component sizes do not match layer tensor layout";
            }
            stop();
            return false;
        }
    ...
    uint64_t storage_per_slot = gate_up_bytes + scale_bytes;
    ...
    const ggml_init_params params = {
        (2 + down_prototypes.size()) * ggml_tensor_overhead() + 1024,
        nullptr,
        true,
    };
    ...
    for (const auto & item : down_prototypes) {
        ...
        auto * tensor = ggml_new_tensor_3d(
                ...
                prototype->ne[1],
                slot_count_);
        ggml_format_name(tensor, "rpp.gemma4.down_cache.%s",
    ...
    if (gate_up_tensor_->nb[2] != gate_up_bytes ||
        ...
        if (error) {
            *error = "page-map component sizes do not match Gemma 4 tensor layout";
        }
        stop();
        return false;
    }
    ggml_set_name(gate_up_tensor_, "rpp.gemma4.gate_up_cache");
    ggml_set_name(down_scale_tensor_, "rpp.gemma4.down_scale_cache");
...
void llama_rpp_gpu_cache::stop() {
    ...
    reclaim_policy_ = llama_rpp_gpu_reclaim_policy::lru;
...
void llama_rpp_gpu_cache::set_reclaim_hints(
        const std::map<llama_rpp_expert_key, double> & hints) {
    std::lock_guard<std::mutex> lock(mutex_);
    reclaim_hints_ = hints;
}
...
int32_t llama_rpp_gpu_cache::reserve_slot_locked(
    ...
    for (int32_t slot = 0; slot < slot_count_; ++slot) {
        ...
        const auto hint = reclaim_hints_.find(key);
        const double score = hint == reclaim_hints_.end() ? 0.0 : hint->second;
        if (reclaim_policy_ == llama_rpp_gpu_reclaim_policy::feo) {
            if (score < victim_score ||
                    (score == victim_score && it->second.last_use < oldest)) {
                victim_score = score;
                oldest = it->second.last_use;
                victim_slot = slot;
            }
        } else if (it->second.last_use < oldest) {
            oldest = it->second.last_use;
            victim_slot = slot;
        }
    }
...
void llama_rpp_gpu_cache::worker_loop(int32_t worker_index) {
    ...
    while (true) {
        ...
        std::string error;
        const bool packed = expected_bytes > 0 &&
        ...
        if (error.empty() && source_->pack_expert_component(
                    current.key.layer,
                    current.key.expert,
                    llama_rpp_expert_component::down_scale,
                    worker.staging_data,
        ...
        const uint64_t copied_bytes = gate_up_bytes + down_bytes + scale_bytes;
