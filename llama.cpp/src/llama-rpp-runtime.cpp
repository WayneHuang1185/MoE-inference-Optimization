...
bool llama_rpp_runtime::configure(const llama_rpp_config & config, std::string * error) {
    ...
    const std::string prefetch_admission =
        config.prefetch_admission ? config.prefetch_admission : "topk";
    if (prefetch_admission != "topk" && prefetch_admission != "feo") {
        if (error) {
            *error = "unsupported RPP prefetch admission policy: " + prefetch_admission;
        }
        return false;
    }
    if (config.feo_min_count <= 0 || config.feo_max_experts < 0 ||
            config.feo_density_threshold < 0.0f || config.feo_density_threshold > 1.0f) {
        if (error) {
            *error = "invalid FEO admission parameters";
        }
        return false;
    }
    ...
    const std::string gpu_reclaim_policy =
        config.gpu_reclaim_policy ? config.gpu_reclaim_policy : "lru";
    if (gpu_reclaim_policy != "lru" && gpu_reclaim_policy != "feo") {
        if (error) {
            *error = "unsupported GPU reclaim policy: " + gpu_reclaim_policy;
        }
        replay_.clear();
        return false;
    }
    ...
    if (gpu_correction == "on") {
        ...
        if (!gpu_cache_.configure(
                    ...
                    gpu_reclaim_policy == "feo"
                        ? llama_rpp_gpu_reclaim_policy::feo
                        : llama_rpp_gpu_reclaim_policy::lru,
                    error)) {
            gpu_transfer_.stop();
            host_prefetcher_.stop();
            replay_.clear();
    ...
    prefetch_admission_ = prefetch_admission;
    gpu_reclaim_policy_ = gpu_reclaim_policy;
    feo_min_count_ = config.feo_min_count;
    feo_max_experts_ = config.feo_max_experts;
    feo_density_threshold_ = config.feo_density_threshold;
    enabled_ = true;
    LLAMA_LOG_INFO(
            "%s: loaded %zu RPP routes (%zu layer predictions), prefill=%d, "
            "decode=%d, depth=%d, top_k=%d, admission=%s, reclaim=%s, "
            "feo_min=%d, feo_density=%.3f, feo_max=%d\n",
            __func__,
            ...
            prefetch_depth_,
            prefetch_top_k_,
            prefetch_admission_.c_str(),
            gpu_reclaim_policy_.c_str(),
            feo_min_count_,
            static_cast<double>(feo_density_threshold_),
            feo_max_experts_);
    return true;
}
...
void llama_rpp_runtime::begin_ubatch(const llama_ubatch & ubatch) {
    ...
    for (uint32_t i = 0; i < ubatch.n_tokens; ++i) {
        ...
        if (prefetch_admission_ == "topk") {
            for (int32_t layer = 0; layer < prefetch_depth_; ++layer) {
                schedule_prefetch(token, layer);
            }
        }
    }
    if (prefetch_admission_ == "feo") {
        update_reclaim_hints(-1);
        for (int32_t layer = 0; layer < prefetch_depth_; ++layer) {
            schedule_ubatch_prefetch(layer);
        }
    }
}
...
bool llama_rpp_runtime::eval_callback(ggml_tensor * tensor, bool ask, void * user_data) {
    ...
    } else if (rpp_requested && is_moe_out) {
        runtime->gpu_cache_.release_layer(parse_layer(tensor->name));
    }
...
bool llama_rpp_runtime::observe_true_route(ggml_tensor * tensor) {
    ...
    if (prefetch_admission_ == "feo") {
        update_reclaim_hints(layer);
        schedule_ubatch_prefetch(layer + prefetch_depth_);
    }
    if (gpu_compute_enabled_) {
...
std::vector<int32_t> llama_rpp_runtime::feo_admitted_experts(int32_t layer) const {
    struct candidate {
        int32_t expert = -1;
        int32_t count = 0;
    ...
    std::map<int32_t, candidate> counters;
    int32_t eligible_tokens = 0;
    for (const auto & token : active_ubatch_) {
        if (layer < 0 || !phase_enabled(token.phase)) {
            continue;
        }
        const llama_rpp_trace_key route_key = {
            token.request_id,
        ...
        for (size_t index = 0; index < selected_count; ++index) {
            const int32_t expert = prediction->experts[index];
            auto & item = counters[expert];
            item.expert = expert;
            ++item.count;
            if (index < prediction->expert_confidences.size()) {
                item.confidence_sum += prediction->expert_confidences[index];
            }
        }
    }
    ...
    const int32_t density_count = std::max<int32_t>(
            ...
            static_cast<int32_t>(std::ceil(
                    static_cast<double>(eligible_tokens) *
                    static_cast<double>(feo_density_threshold_))));
    const int32_t required_count = std::max(feo_min_count_, density_count);

    std::vector<candidate> candidates;
    for (const auto & item : counters) {
        if (item.second.count >= required_count) {
    ...
    std::sort(candidates.begin(), candidates.end(), [](const candidate & lhs, const candidate & rhs) {
        ...
        if (lhs.confidence_sum != rhs.confidence_sum) {
            return lhs.confidence_sum > rhs.confidence_sum;
        }
        return lhs.expert < rhs.expert;
    });
    if (feo_max_experts_ > 0 &&
            candidates.size() > static_cast<size_t>(feo_max_experts_)) {
        candidates.resize(static_cast<size_t>(feo_max_experts_));
    }

    std::vector<int32_t> result;
    result.reserve(candidates.size());
    for (const auto & item : candidates) {
        result.push_back(item.expert);
    }
    return result;
}

void llama_rpp_runtime::schedule_ubatch_prefetch(int32_t layer) {
    if (prefetch_depth_ <= 0 || layer < 0 || active_ubatch_.empty()) {
        return;
    }
    const auto selected_experts = feo_admitted_experts(layer);
    if (selected_experts.empty()) {
        return;
    }

    const llama_rpp_token_metadata * representative = nullptr;
    for (const auto & token : active_ubatch_) {
        if (phase_enabled(token.phase)) {
...
void llama_rpp_runtime::update_reclaim_hints(int32_t current_layer) {
    if (gpu_reclaim_policy_ != "feo" || !gpu_cache_.enabled()) {
        return;
    }
    std::map<llama_rpp_expert_key, double> hints;
    const int32_t start = std::max(0, current_layer + 1);
    const int32_t stop = std::max(start, current_layer + 1 + std::max(1, prefetch_depth_));
    for (int32_t layer = start; layer < stop; ++layer) {
        const auto experts = feo_admitted_experts(layer);
        const double distance_weight = 1.0 / static_cast<double>(1 + layer - current_layer);
        for (int32_t expert : experts) {
            hints[{layer, expert}] += distance_weight;
        }
    }
    gpu_cache_.set_reclaim_hints(hints);
}
...
void llama_rpp_runtime::write_trace_event(
    ...
    std::vector<int32_t> prefetched_experts;
    if (prefetch_admission_ == "feo") {
        prefetched_experts = feo_admitted_experts(layer);
    } else {
        const size_t prefetched_count = std::min(
                predicted_experts.size(),
                static_cast<size_t>(prefetch_top_k_));
        prefetched_experts.assign(
    ...
    json event = {
        ...
        {"prefetch_admission", prefetch_admission_},
        {"gpu_reclaim_policy", gpu_reclaim_policy_},
        {"feo_min_count", feo_min_count_},
        {"feo_density_threshold", feo_density_threshold_},
        {"feo_max_experts", feo_max_experts_},
