...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--rpp-scheduler"}, "{none,fifo,active_expert_union_rebatch,oracle-window,decode_rpp_top8_mmap_ubatch}",
        "experimental RPP live scheduler mode (default: none)",
        [](common_params & params, const std::string & value) {
            params.rpp_scheduler = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-live-model"}, "PATH",
        "experimental RPP TorchScript model path (accepted for live scheduler compatibility)",
        [](common_params & params, const std::string & value) {
            params.rpp_live_model = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-tensor-ranges"}, "CSV",
        "experimental RPP tensor range CSV for expert prefetch",
        [](common_params & params, const std::string & value) {
            params.rpp_tensor_ranges = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-oracle-trace"}, "JSONL",
        "experimental RPP oracle-window trace JSONL",
        [](common_params & params, const std::string & value) {
            params.rpp_oracle_trace = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-window-size"}, "N",
        string_format("experimental RPP oracle-window FIFO size (default: %d)", params.rpp_window_size),
        [](common_params & params, int value) {
            if (value < 1) {
                throw std::invalid_argument("rpp window size must be positive");
            }
            params.rpp_window_size = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-prefetch-count-threshold"}, "N",
        string_format("experimental RPP oracle-window counter threshold (default: %d)", params.rpp_prefetch_count_threshold),
        [](common_params & params, int value) {
            if (value < 1) {
                throw std::invalid_argument("rpp prefetch count threshold must be positive");
            }
            params.rpp_prefetch_count_threshold = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-reclaim-mode"}, "{trace-priority,log-only,fadvise-dontneed,off}",
        "experimental RPP reclaim mode; v1 defaults to trace-only priority candidates",
        [](common_params & params, const std::string & value) {
            if (value != "trace-priority" && value != "log-only" && value != "fadvise-dontneed" && value != "off") {
                throw std::invalid_argument("unsupported rpp reclaim mode");
            }
            params.rpp_reclaim_mode = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-max-decode-slots"}, "N",
        "experimental RPP maximum decode slots (accepted for scheduler compatibility)",
        [](common_params & params, int value) {
            params.rpp_max_decode_slots = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-predict-topk"}, "N",
        "experimental RPP predictor top-k (accepted for scheduler compatibility)",
        [](common_params & params, int value) {
            params.rpp_predict_topk = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-cache-threshold"}, "P",
        "experimental RPP cache threshold (accepted for scheduler compatibility)",
        [](common_params & params, const std::string & value) {
            params.rpp_cache_threshold = std::stof(value);
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-page-stride"}, "N",
        "experimental RPP page stride (accepted for scheduler compatibility)",
        [](common_params & params, int value) {
            params.rpp_page_stride = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-predictor-threads"}, "N",
        "experimental RPP predictor threads (accepted for scheduler compatibility)",
        [](common_params & params, int value) {
            params.rpp_predictor_threads = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--rpp-predictor-cpu-mask"}, "MASK",
        "experimental RPP predictor CPU mask (accepted for scheduler compatibility)",
        [](common_params & params, const std::string & value) {
            params.rpp_predictor_cpu_mask = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
