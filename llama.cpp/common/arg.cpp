...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        string_format("RPP prefetch admission policy (default: %s)", params.rpp.prefetch_admission.c_str()),
        [](common_params & params, const std::string & value) {
            if (value != "topk" && value != "feo") {
                throw std::invalid_argument("invalid RPP prefetch admission policy");
            }
            params.rpp.prefetch_admission = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_RPP_PREFETCH_ADMISSION"));
    add_opt(common_arg(
        {"--rpp-feo-min-count"}, "N",
        string_format("minimum ubatch-level predicted count required for FEO admission (default: %d)",
            params.rpp.feo_min_count),
        [](common_params & params, int value) {
            if (value <= 0) {
                throw std::invalid_argument("RPP FEO min count must be positive");
            }
            params.rpp.feo_min_count = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_RPP_FEO_MIN_COUNT"));
    add_opt(common_arg(
        {"--rpp-feo-max-experts"}, "N",
        string_format("maximum experts admitted per ubatch/layer by FEO, 0 = unlimited (default: %d)",
            params.rpp.feo_max_experts),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("RPP FEO max experts must be non-negative");
            }
            params.rpp.feo_max_experts = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_RPP_FEO_MAX_EXPERTS"));
    add_opt(common_arg(
        {"--rpp-feo-density-threshold"}, "N",
        string_format("minimum ubatch-level density required for FEO admission (default: %.3f)",
            (double) params.rpp.feo_density_threshold),
        [](common_params & params, const std::string & value) {
            const float parsed = std::stof(value);
            if (parsed < 0.0f || parsed > 1.0f) {
                throw std::invalid_argument("RPP FEO density threshold must be in [0, 1]");
            }
            params.rpp.feo_density_threshold = parsed;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_RPP_FEO_DENSITY_THRESHOLD"));
    add_opt(common_arg(
        {"--rpp-page-map"}, "FILE",
    ...
    add_opt(common_arg(
        ...
        string_format("RPP GPU expert cache reclaim policy (default: %s)", params.rpp.gpu_reclaim_policy.c_str()),
        [](common_params & params, const std::string & value) {
            if (value != "lru" && value != "feo") {
                throw std::invalid_argument("invalid RPP GPU reclaim policy");
            }
            params.rpp.gpu_reclaim_policy = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_RPP_GPU_RECLAIM_POLICY"));
    add_opt(common_arg(
        {"--rpp-prefill"},
        {"--no-rpp-prefill"},
