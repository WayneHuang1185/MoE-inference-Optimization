...
// PDL requires CUDA device APIs that are available in newer CUDA toolkits.
// Keep this disabled for CUDA 11.8 builds used by this project; otherwise nvcc
// fails on cudaGridDependencySynchronize/cudaTriggerProgrammaticLaunchCompletion.
// __CUDA_ARCH__  is undefined in host passes; GPU arch check happens in device-side code.
#if !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA) && CUDART_VERSION >= 12030
#    define GGML_CUDA_USE_PDL
#endif  // !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA) && CUDART_VERSION >= 12030
