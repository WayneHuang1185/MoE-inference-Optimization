#!/usr/bin/env bash
# Runtime llama-server probe with optional RPP page-cache prefetch sidecar.
set -euo pipefail
...
STATISTICS_DIR="${STATISTICS_DIR:-$OUT_DIR/statistics/runtime_rpp_prefetch_${RUN_TIMESTAMP}}"
...
PREFETCH_MODE="${PREFETCH_MODE:-none}"
...
CONTAINER_NAME="gemma4-runtime-rpp-${RUN_TIMESTAMP}-${PORT}"
SERVER_LOG="$STATISTICS_DIR/server.log"
PROBE_LOG="$STATISTICS_DIR/runtime_rpp_prefetch_probe.log"
...
docker run -d \
  ...
  bash -lc '
set -euo pipefail
mkdir -p /tmp/llama_slots_runtime_rpp
python3 experiments/gemma4_IO_behaviors/mem48G/drop_file_cache.py "$MODEL" >/tmp/runtime_rpp_drop_file_cache.log 2>&1 || true
...
LLAMA_CMD=(
  ...
  --no-warmup \
  --slots \
  --slot-save-path /tmp/llama_slots_runtime_rpp \
...
docker run --rm \
  ...
  bash \
  python3 "$UTIL_DIR/runtime_rpp_prefetch_probe.py" \
...
docker logs "$CONTAINER_NAME" > "$SERVER_LOG" 2>&1 || true
cp "$STATISTICS_DIR/REPORT.md" "$OUT_DIR/REPORT.md"
echo "runtime RPP prefetch statistics: $STATISTICS_DIR"
